...
export function CommentsModal({
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="mt-1 flex items-center gap-4 text-xs text-slate-500 dark:text-slate-400 flex-wrap">
            ...
            {likesModal.open && likesModal.id && (
              <div className="fixed inset-0 bg-black/40 dark:bg-black/60 flex items-center justify-center z-50" onClick={(e) => {
                  e.stopPropagation(); 
                  setLikesModal({ open: false });
                }}>
                <div className="bg-white dark:bg-slate-800 w-full max-w-md rounded-2xl shadow-lg p-5 relative" onClick={(e) => e.stopPropagation()}>
                  <button
                    onClick={() =>
                      setLikesModal({
                        open: false,
            ...
            <div className="mt-2 flex gap-3 w-full">
              ...
              <div className="flex-1 flex flex-col gap-1 min-w-0">
                {!repliesMap[id] ? (
                  ...
                  <div className="text-xs text-slate-500 dark:text-slate-400 py-2">No replies yet</div>
                ) : (
          ...
          {replyingTo === (parentId ?? id) && !parentId && (
            <div className="mt-3 w-full">
              <div className="text-xs text-slate-500 dark:text-slate-400 mb-2">
                Replying to{" "}
                <span className="font-semibold text-slate-700 dark:text-slate-300">@{replyingToName}</span>
              </div>
  ...
  return (
    <div className="fixed inset-0 z-50">
      <div className="absolute inset-0 bg-black/50 dark:bg-slate-900/80 backdrop-blur-sm transition-all" onClick={onClose} />
      ...
      <div className="absolute left-1/2 top-1/2 w-[94%] max-w-lg -translate-x-1/2 -translate-y-1/2 rounded-2xl bg-white dark:bg-slate-800 shadow-2xl overflow-hidden border border-transparent dark:border-slate-700/50">
        <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100 dark:border-slate-700/50">
          <div className="w-8" />
          <div className="font-semibold text-slate-800 dark:text-slate-100">Comments</div>
        ...
        <div className="max-h-[60vh] overflow-y-auto px-4 py-4 space-y-5 custom-scrollbar">
          {loading ? (
            ...
            <div className="text-center text-slate-500 dark:text-slate-400">No comments yet</div>
          ) : (
