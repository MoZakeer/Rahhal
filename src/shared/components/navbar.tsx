  ...
  return (
    <header className={`fixed top-0 z-40 w-full bg-white/90 dark:bg-slate-900/90 backdrop-blur-md border-b border-slate-100 dark:border-slate-800 shadow-sm transition-transform duration-500 ease-in-out ${isNavVisible ? "translate-y-0" : "-translate-y-full"}`}>
        ...
        <div className="flex items-center gap-2 ml-auto lg:gap-3">
          {hasToken ? (
              ...
              <Popover className="relative hidden lg:block">
                <Popover.Button className="flex items-center gap-2 p-1 pr-3 rounded-full hover:bg-slate-50 dark:hover:bg-slate-800 transition-all outline-none border border-transparent hover:border-slate-100 dark:hover:border-slate-700 group">
                  <img
                    ...
                    alt="user"
                  />
                  <p className="text-sm font-bold text-indigo-600 dark:text-indigo-400">{profile?.userName || "Account"}</p>
            ...
            <div className="hidden lg:flex items-center gap-3">
              <Link to="/login" className="px-4 py-2 text-sm font-bold text-slate-600 dark:text-slate-300 hover:text-indigo-600 dark:hover:text-indigo-400">Sign In</Link>
              <Link to="/sign-up" className="rounded-xl bg-indigo-600 dark:bg-indigo-500 px-5 py-2 text-sm font-bold text-white shadow-lg">Sign Up</Link>
            </div>
          )}

          {/* Mobile Menu Button */}
          <button onClick={() => setMobileOpen(true)} className="lg:hidden p-2 rounded-xl bg-slate-50 dark:bg-slate-800 text-slate-600 dark:text-slate-300 transition-colors hover:bg-slate-100 dark:hover:bg-slate-700">
      ...
      <Dialog open={mobileOpen} onClose={setMobileOpen} className="relative z-50 lg:hidden">
        <DialogBackdrop transition className="fixed inset-0 bg-slate-900/60 dark:bg-slate-950/80 backdrop-blur-sm transition-opacity duration-300 data-[closed]:opacity-0" />
        <div className="fixed inset-0 flex justify-end">
          <DialogPanel transition className="relative flex h-full w-full max-w-sm flex-col bg-white dark:bg-slate-900 px-6 py-6 shadow-2xl transition duration-300 data-[closed]:translate-x-full">
            <div className="mb-8 flex items-center justify-between">
              <span className="text-xl font-black text-slate-800 dark:text-slate-100">Menu</span>
              <button onClick={() => setMobileOpen(false)} className="p-2 rounded-full bg-slate-50 dark:bg-slate-800 text-slate-600 dark:text-slate-300"><X className="h-5 w-5" /></button>
            </div>
            ...
            <div className="flex flex-col gap-2 flex-1 overflow-y-auto">
              ...
              {hasToken && (
                ...
                >
                  <img src={profile?.profilePicture ? `${API_BASE_URL}${profile.profilePicture}` : "/api/placeholder/40/40"} className="h-12 w-12 rounded-full object-cover border-2 border-white dark:border-slate-800 shadow-sm" />
                  <div>
                    <p className="font-black text-slate-800 dark:text-slate-100 leading-none">{profile?.userName}</p>
                    <p className="text-xs font-bold text-indigo-600 dark:text-indigo-400 mt-1">View My Profile</p>
                  </div>
                </Link>
              )}
              ...
              {/* استبدلنا الزرار اليدوي بالـ Component بتاعنا عشان يشتغل في الموبايل كمان */}
              <ThemeToggleButton />
              
              <button className="flex items-center gap-4 p-4 text-slate-600 dark:text-slate-300 font-bold hover:bg-slate-50 dark:hover:bg-slate-800 rounded-xl transition-colors cursor-pointer"><Languages className="h-6 w-6" /> Arabic Language</button>
            </div>
            ...
            <div className="mt-auto pt-6 border-t border-slate-100 dark:border-slate-800">
              {hasToken ? (
                ...
                <div className="flex flex-col gap-3">
                  <Link to="/login" onClick={() => setMobileOpen(false)} className="flex justify-center rounded-xl bg-slate-100 dark:bg-slate-800 p-4 font-bold text-slate-700 dark:text-slate-200">Sign In</Link>
                  <Link to="/sign-up" onClick={() => setMobileOpen(false)} className="flex justify-center rounded-xl bg-indigo-600 dark:bg-indigo-500 p-4 font-bold text-white shadow-lg">Sign Up</Link>
                </div>
              )}
            </div>
          </DialogPanel>
