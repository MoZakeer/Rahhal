...
const TripCard = ({ trip, onToggleFavorite }: TripCardProps) => {
  ...
  return (
    <Link to={`/trip/${trip.id}`} className="group block w-full">
      {/* التعديل الأساسي هنا:
        الموبايل: h-auto و flex-col
        الديسكتوب (md): h-[420px] عشان نحافظ على شكل المربع الفني
      */}
      <div className="relative flex h-auto md:h-[420px] w-full flex-col overflow-hidden rounded-3xl bg-white shadow-sm md:transition-all md:duration-500 md:ease-out md:group-hover:-translate-y-2 md:group-hover:shadow-[0_20px_50px_rgba(37,99,235,0.08)] border border-slate-100 md:border-none">
        ...
        <div className="relative h-[240px] w-full shrink-0 overflow-hidden bg-slate-50 md:absolute md:inset-0 md:h-full">
          ...
          {/* Gradient مخصص للديسكتوب عشان يدمج الصورة مع الكارد الأبيض */}
          ...
          {/* Gradient خفيف للموبايل تحت عشان يبرز أيقونة نسبة التطابق لو موجودة */}
          ...
          <Button
            variant="ghost"
            size="icon"
            className="absolute right-4 top-4 md:right-5 md:top-5 h-10 w-10 rounded-full bg-white/90 backdrop-blur-md shadow-sm border border-white/50 hover:bg-white transition-all duration-300 z-10"
            onClick={(e) => {
              e.preventDefault();
