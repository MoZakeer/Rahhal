...
export default function FloatingSidebar() {
  ...
  const [unReadMessagesCount, setUnReadMessageCount] = useState<number>();
    ...
    <div className="lg:hidden">
      ...
      <AnimatePresence>
        {!isOpen && (
          ...
          >
            <div
              className={`absolute top-1/2 -translate-y-1/2 w-1 h-6 bg-blue-600/60 rounded-full ${isRTL ? "left-0.5" : "right-0.5"}`}
            />
          </motion.div>
        )}
      </AnimatePresence>
        ...
        {isOpen && (
          <>
            ...
            <motion.div
              ...
              className={`fixed top-[20%] w-12 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 shadow-2xl py-4 flex flex-col items-center gap-4 z-60 ${
              ...
              {menuItems.map((item) => (
                <div key={item.path} className="relative flex items-center">
                  <Link
                    ...
                    onTouchStart={() =>
                      handleTouchStart(t(`navbar.${item.labelKey}`))
                    }
                    onTouchEnd={handleTouchEnd}
                    onMouseEnter={() =>
                      setActiveTooltip(t(`navbar.${item.labelKey}`))
                    } // للـ Desktop
                  ...
                  >
                    ...
                    >
                      <item.icon
                        className={`h-5 w-5 ${item.isAI && location.pathname !== item.path ? "text-amber-500" : ""}`}
                      />
                    </motion.div>
                  </Link>
                  ...
                  <AnimatePresence>
                    {activeTooltip === t(`navbar.${item.labelKey}`) && (
                      <motion.div
                        ...
                        className={`absolute pointer-events-none px-3 py-1.5 rounded-lg bg-slate-800 dark:bg-slate-700 text-white text-[11px] font-bold shadow-xl whitespace-nowrap z-70 ${
                          isRTL ? "left-14" : "right-14"
                        }`}
                      >
                        {activeTooltip}
                        <div
                          className={`absolute top-1/2 -translate-y-1/2 w-2 h-2 rotate-45 bg-slate-800 dark:bg-slate-700 ${
