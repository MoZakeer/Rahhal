...
const JoinRequestsSection = ({ requests, hideHeader = false, loading = false, onStatusChange }: Props) => {
  ...
  // console.log("JoinRequestsSection rendered with requests:", requests);
  ...
  return (
    <div>
      ...
      <div className={hideHeader ? "space-y-3" : "mt-4 space-y-3"}>
        ...
        {/*  TO DO - DISPLAY USER IMAGE AND ONCE USER CLICK GO TO ITS PROFILE PAGE ON RAHHAL */}
        {requests.map((r) => (
          <div key={r.id} className="rounded-lg border border-gray-300 dark:border-slate-800 bg-card dark:bg-slate-900 p-4 shadow-card">
            <div className="flex items-start gap-3">
              ...
              <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-primary text-sm font-bold text-primary-foreground dark:text-white">
                {r.userAvatar}
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex flex-wrap items-center gap-2">
                  <p className="font-medium dark:text-slate-200">{r.userName}</p>
